...
import {
	...
	CompleteContainer, ContainerWHorizControls, transitionStyles, overlayTransition, transitionStyles2
} from './NewCardStyles'
...
export default class NewFormCard extends PureComponent {
	...
	render() {
		...
		return (
			<Fragment>
				<Transition in={this.state.expand} timeout={350}>
					{state => {
						console.log(state)
						return <CompleteContainer style={{ ...overlayTransition[state] }} expand={expand}>
							<ContainerWHorizControls expand={expand} style={{ ...transitionStyles[state] }}>
								<FormContainer style={{ ...transitionStyles2[state] }} innerRef={this.setExpandedCardRef} expand={expand} id={this.props.id} horizOpen={horizOpen}>
									<FormImg>
