...
class HelpList extends Component {
	...
	// 	// 1. Make a function inside HelpNew , call it helpIdGenerator or whatever and generate an readable id and set it in the state 
	// 	// and pass it to the server togheter with locale_content 
	// 	// {help_id: *your_generated_string* , locale_content: {etc.}}
	// 	// 2. Make an input field that gives the user the possibility to fill it itself then set it in the state and 
	// 	// pass it to the server togheter with locale_content 
	// 	// {help_id: *your_generated_string* , locale_content: {etc.}}

	// 	// How the function to call the server should look like
	// 	// var callServer = await updateHelpItem(helpItem)
	// 	//where callServer returns true/false/error, updateHelpItem is a function in Data file that calls the server using axios
	// 	// ( PUT function)
	...
	// 		3. Then make the function on the server so that PUT-ing (HTTP Call) an helpItem to the server 
	// 		on http://localhost:port/updatehelpitem will find the item based on its help_id, and update it
	...
	render() {
		...
		return (
			<div>
				...
				<br></br>
				<HelpModal showModal={this.state.showModal} openModal={this.openModal}>
					<HelpNew helpItem={this.state.openHelp} submit={this.updateHelpItem}/>
				</HelpModal>
			</div>
		)
	}
